...
echo "Vuoi accedere al sito? [S] [N]"
read response
...
echo "Vuoi inviarmi le tue informazioni, specificate dal garante della privacy, per accedere al sito? [S] [N]"
read response
if [ $response == "S" ] || [ $response == 's' ]; then
...
if [ $root == $value ]; then
    ...
    #scrivo la chiave pubblica sulla blockchain
    echo "----------------------------------" >> "../blockchain.txt"
    echo $extracted_key >> "../blockchain.txt"
    ...
    if ! grep -q "$extracted_key" "../Server/Database.txt"; then
        echo "Dovresti prima registrarti al sito!"
        rm "../Server/Informazioni_$1.txt"
        sudo rm "../Server/Informazioni_$1.bin"
        exit 1
    fi

    pubkey=$(sed -n '/^pubkey/,/^ban/p' "../Server/Database.txt")
    
    saldo=$(grep "saldo:" "../Server/Database.txt")
    ban=$(grep "ban:" "../Server/Database.txt")

    nline=$(grep -n "$extracted_key" "../Server/Database.txt" | cut -d ':' -f1 | tr -d ' ')
    IFS=$'\n' read -r first_element _ <<< "$nline"
    
    extracted_lines=$(awk -v start=$first_element 'NR >= start && NR <= start+4' "../Server/Database.txt")
